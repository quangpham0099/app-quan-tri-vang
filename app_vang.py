...
st.sidebar.header("⚙️ Quy Mô & Nguồn Vốn")

# 1. Khối quy mô đầu tư
total_gold = st.sidebar.number_input("Số lượng vàng mua (Cây)", value=4.5, step=0.1, format="%.2f")
buy_price = st.sidebar.number_input("Giá mua vào (Triệu/cây)", value=147.0, step=1.0, format="%.2f")

total_cost = total_gold * buy_price
st.sidebar.success(f"**👉 Tổng giá trị đầu tư: {total_cost:,.2f} Tr**")

# 2. Khối cơ cấu vốn
loan = st.sidebar.number_input("Vốn vay ngân hàng (Triệu VNĐ)", value=661.5, step=10.0, format="%.2f")
own_capital = total_cost - loan

if own_capital >= 0:
    st.sidebar.info(f"**Vốn tự có cần bỏ ra: {own_capital:,.2f} Tr**")
else:
    st.sidebar.warning(f"**Tiền dư ra sau khi vay: {-own_capital:,.2f} Tr**")
...
st.sidebar.markdown("---")
st.sidebar.header("⏱️ Chiến Lược Chốt Lời")
...
st.sidebar.markdown(f"*Tỷ lệ bán đợt cuối tự động tính: **{r3:.1f}%***")
...
if total_gold > 0:
    ...
    # Dòng tiền cuối cùng thu về (sau khi đã dập nợ tự động trong các đợt)
    final_cash_in_hand = sav3 - prin3
    
    # Lợi nhuận ròng = Tổng tiền cầm về - Vốn tự có ban đầu
    net_profit = final_cash_in_hand - own_capital
    
    # Giá quy đổi = (Lợi nhuận ròng + Tổng chi phí đầu tư gốc) / Tổng số vàng
    equivalent_price = (net_profit + total_cost) / total_gold
    
    total_revenue = rev1 + rev2 + rev3
    actual_avg_price = total_revenue / total_gold
    ...
    df = pd.DataFrame({
        ...
        "Bán ra (Cây)": [round(q1, 2), round(q2, 2), round(q3, 2)],
    ...
    excel_data = {
        "Thông số": ["Tổng số vàng (Cây)", "Giá mua", "Tổng đầu tư", "Vốn vay", "Vốn tự có", "Lãi vay %", "Đợt 1 (T/Tỷ lệ/Giá)", "Đợt 2", "Đợt 3", "Lợi nhuận ròng", "Giá quy đổi"],
        "Giá trị": [total_gold, buy_price, total_cost, loan, own_capital, loan_rate, f"T{m1} / {r1}% / {p1}", f"T{m2} / {r2}% / {p2}", f"T{m3} / {r3}% / {p3}", round(net_profit,2), round(equivalent_price,2)]
    }
